...
USE SCHEMA BRONZE;
-------------------------------------------------------------------------------------------
    -- PROBLEM: There are 12 items referred to by SILVER.INTERVALS ITEM_0 -> ITEM_6
    -- that do not have corresponding reference rows under SILVER.ITEMS_REF
-------------------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------------------
    -- SOLUTION: Manually discover and seed the 12 missing entries into BRONZE.ITEMS_REF
...
WHERE NOT EXISTS (
    SELECT 1
    FROM BRONZE.ITEMS_REF AS tgt
    WHERE tgt.ITEM_ID = v.ITEM_ID
);

-------------------------------------------------------------------------------------------
    -- VERIFY: Verify that BRONZE.ITEMS_REF have received the change and all downstream
    -- propagations happen for this schema (SILVER.ITEMS_REF)
-------------------------------------------------------------------------------------------
--SELECT
