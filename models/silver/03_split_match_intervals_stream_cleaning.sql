...
WITH base AS (
    SELECT
        ID,
        -- Normalize match id casing/whitespace, consistent with the other silver tables
        ...
        CASE WHEN ((PLAYER_ID - 1) % 10) + 1 BETWEEN 1 AND 5 THEN 'Blue' ELSE 'Red' END
                                                 AS TEAM,
        -- Snap to the nearest 5-minute mark; raw values are mostly exact but have edge noise
        ROUND(MINUTE / 5.0) * 5                 AS MINUTE,
        -- Economy + KDA stats pass through untouched; imputation happens downstream
        CURRENT_GOLD, TOTAL_GOLD, CS, JUNGLE_CS, XP, LEVEL,
        KILLS, DEATHS, ASSISTS,
        -- Item id 0 means "no item" -- normalize to NULL before the ref lookup
        NULLIF(ITEM_0, 0) AS ITEM_0, NULLIF(ITEM_1, 0) AS ITEM_1, NULLIF(ITEM_2, 0) AS ITEM_2,
        NULLIF(ITEM_3, 0) AS ITEM_3, NULLIF(ITEM_4, 0) AS ITEM_4, NULLIF(ITEM_5, 0) AS ITEM_5,
        NULLIF(ITEM_6, 0) AS ITEM_6,
        -- Team objective counters, pass through untouched
        ...
        -- Signed diff columns, pass through untouched
...
imputed AS (
    SELECT
        ...
        IMPUTE_WITH_AVG(b.GOLD_DIFF, a.AVG_GOLD_DIFF, -9999)                 AS GOLD_DIFF,
        IMPUTE_WITH_AVG(b.XP_DIFF, a.AVG_XP_DIFF, -9999)                     AS XP_DIFF,
        IMPUTE_WITH_AVG(b.TEAM_GOLD_DIFF, a.AVG_TEAM_GOLD_DIFF, -9999)       AS TEAM_GOLD_DIFF
    FROM base b
    JOIN average_calc a ON a.MINUTE = b.MINUTE
),
...
item_names AS (
    SELECT
        x.ID,
        r0.ITEM_NAME AS ITEM_0, r1.ITEM_NAME AS ITEM_1, r2.ITEM_NAME AS ITEM_2,
        r3.ITEM_NAME AS ITEM_3, r4.ITEM_NAME AS ITEM_4, r5.ITEM_NAME AS ITEM_5,
...
SELECT
    ...
    i.CS, i.JUNGLE_CS, i.XP, i.LEVEL, i.KILLS, i.DEATHS, i.ASSISTS,
